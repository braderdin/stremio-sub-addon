...
# Import modul konfigurasi asal & modul pekerja Camoufox V3 sedia ada
b2_config = importlib.import_module("00_b2_config")
browser_worker_v3 = importlib.import_module("02_b2_browser_worker_v3")

# Import modul tugas kecil Chromium baharu
step01_chromium = importlib.import_module("02_b2_step01_chromium")
...
def main():
    ...
    print(f"\n=======================================================")
    print(f"⭐ [SUITE V3 HYBRID] Memulakan Automasi bagi {total_accounts} Akaun")
    print(f"=======================================================\n")
    ...
    for acc in b2_config.ACCOUNTS_QUEUE:
        ...
        try:
            # -------------------------------------------------------------
            # 1. JEDA MULA SKRIP UTAMA (5 - 10 SAAT)
            # -------------------------------------------------------------
            warmup_seconds = random.randint(6, 9)
            print(f"⏳ [SKRIP UTAMA BERMULA] Persediaan sistem selama {warmup_seconds} saat...")
            for remaining in range(warmup_seconds, 0, -1):
                print(f"   Memulakan Chromium dalam masa {remaining} saat...", end="\r")
                time.sleep(1)
            print(f"\n[✓] Persediaan selesai!\n")

            # -------------------------------------------------------------
            # 2. PELAYAR 1 (CHROMIUM): DAFTAR EMEL & TEKAN GET STARTED
            # -------------------------------------------------------------
            step01_chromium.run_step01_chromium(email)

            # -------------------------------------------------------------
            # 3. PELAYAR 2 (CAMOUFOX): MENJALANKAN KOD ASAL V3 SEPENUHNYA
            #    (Buka Github/Neocities, Papar Banner Emel, Pantau Gmail,
            #     Step 2 Setup, Step 3 2FA, Step 4 Bucket, Step 5 App Keys)
            # -------------------------------------------------------------
            print(f"\n🦊 [PELAYAR 2: CAMOUFOX] Memulakan Pekerja V3 Asal...")
            result = browser_worker_v3.process_b2_account(acc)
            result["acc_num"] = acc_num

            # Simpan rekod fail JSON individu
            with open(single_file, "w", encoding="utf-8") as f:
                json.dump(result, f, indent=4)
            print(f"[+] Fail individu disimpan: {single_file}")

            # Kemas kini fail JSON gabungan
