#!/usr/bin/env python3
# ==============================================================================
# PROJEK: STREMIO SUBTITLE CATALOG - RESOLVER TEST ENGINE V23 (ZERO-FALSE-POSITIVE LOCKDOWN)
# LOKASI: /home/braderdin/stremio-sub-addon/ondemand_title_packs/experiments/test_malay_300_resolver_v23.py
# ==============================================================================
...
OUTPUT_JSON_PATH = TEMP_DIR / "sample_malay_v23_300_results.json"
OUTPUT_DB_PATH = TEMP_DIR / "sample_malay_v23_300_results.db"
...
# ==============================================================================
# PENGECAMAN AWAL EPISOD & TIPE MEDIA V23
# ==============================================================================
def parse_episode_and_season_early_v23(sub_fn: str, slug: str, pkg: str) -> Tuple[Optional[int], Optional[int], str]:
    ...
    # 3. [BAHARU V23] Kod Singkatan Drama (cth: "SHR12.ass", "DOTS05.srt")
...
# ==============================================================================
# PEMBINAAN QUERY HIERARKI V23 (SINGLE-WORD LOCKDOWN & CLEAN FILENAME PRIORITY)
# ==============================================================================
def build_hierarchical_queries_v23(slug: str, pkg: str, sub_fn: str, int_p: str, media_type: str) -> Tuple[List[Tuple[str, str]], Optional[int]]:
    ...
    def add_q(q_text: str, tier: str):
        ...
        words = clean_q.split()
        # Buang sebarang perkataan kata sendi di hujung query (cth: "alice in" -> "alice")
        while words and words[-1].lower() in STOPWORDS:
            words.pop()
        clean_q = " ".join(words).strip()

        # [KUNCI KESELAMATAN 1 - V23]: Sekiranya slug asal >= 3 patah perkataan,
        # JANGAN SESEKALI benarkan query dipotong sehingga tinggal 1 perkataan tunggal!
        ...
        if fn_clean:
            ...
            if any(not w.isdigit() for w in words_fn):
                ...
                for candidate_fn in target_fn_titles:
                    ...
                    # Syarat: Mesti ada sekurang-kurangnya 1 perkataan teras bertindih dengan slug
                    if slug_core_tokens and not fn_core.intersection(slug_core_tokens):
                        continue
                    add_q(candidate_fn, "2_SUB_FILENAME")

    # -------------------------------------------------------------------------
    # TIER 3: SLUG SHORT CHUNKS (3-Perkataan & 2-Perkataan Sahaja)
    # -------------------------------------------------------------------------
    for part in alias_parts:
        part_clean = re.sub(r"\b(19\d\d|20\d\d)\b", " ", part)
        part_text = sanitize_general_text(part_clean)
...
# ==============================================================================
# ENJIN PENILAIAN INTEGRITI V23 (ZERO-FALSE-POSITIVE SECURE)
# ==============================================================================
def verify_candidate_integrity_v23(query: str, cand_title: str, query_year: Optional[int],
    ...
    # 8. Kawalan Sekuel Bebas Tahun (V23)
    ...
    # 9. Skor Keserupaan RapidFuzz & [KUNCI KESELAMATAN 2 - V23]:
    # Menghentikan Ralat 'Last Hero' vs 'Last Action Hero'
    ...
    # Penentuan Ambang Ketat V23
    if is_ultra_generic:
        min_threshold = 0.88
    elif len(q_core) <= 2:
        # Jika query pendek (<= 2 perkataan) dan calon menambah perkataan teras lain (cth: Action Hero),
        # ambang WAJIB tinggi (0.88) agar perkataan luar tidak diserap secara salah
    ...
    audit_meta["reason"] = "LULUS SEMUA BENTENG INTEGRITI V23"
...
def query_cinemeta_catalog(query_title: str, media_type: str) -> List[Dict[str, Any]]:
    ...
    headers = {"User-Agent": "Mozilla/5.0 (StremioMalayResolverV23/1.0)"}
    try:
...
# ==============================================================================
# ALUR KERJA RESOLVER V23
# ==============================================================================
def resolve_single_sample_v23(item: Dict[str, Any]) -> Dict[str, Any]:
    ...
    season, episode, media_type = parse_episode_and_season_early_v23(sub_fn, slug, pkg)
    queries_with_tiers, year = build_hierarchical_queries_v23(slug, pkg, sub_fn, int_p, media_type)
    ...
    for q_text, tier in queries_with_tiers:
        ...
        for cand in metas[:6]:
            ...
            valid, score, audit_meta = verify_candidate_integrity_v23(
...
def save_v23_sqlite(results: List[Dict[str, Any]]):
    ...
    cur = conn.cursor()
    cur.execute("""
        CREATE TABLE sample_v23_results (
    ...
    cur.executemany("""
        INSERT INTO sample_v23_results 
...
def main():
    console.print("\n" + "=" * 80)
    console.print("🔬 [bold green]UJIAN PENYELESAIAN 300 SAMPEL NULL V23 (ZERO-FALSE-POSITIVE LOCKDOWN)[/bold green]")
    console.print("=" * 80 + "\n")
    ...
    console.print(f"🛡️  Benteng V23: [green]Single-Word Guard + Clean Filename Priority + 0.88 Short-Threshold[/green]\n")
    ...
    with Progress(
        ...
        task = progress.add_task("Mengesahkan IMDb V23...", total=len(samples))

        with ThreadPoolExecutor(max_workers=WORKER_THREADS) as executor:
            future_to_item = {executor.submit(resolve_single_sample_v23, s): s for s in samples}
    ...
    table = Table(title="📊 Keputusan Ujian Resolusi 300 Sampel NULL (Formula V23)", border_style="cyan")
